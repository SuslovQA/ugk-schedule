...
import ru.ugk.schedule.controller.api.MaxMiniAppController;
...
class MaxLaunchDataServiceTest {
    private final MaxLaunchDataService validator = new MaxLaunchDataService("test-token", new ObjectMapper());
    ...
    void rejectsTamperingDuplicatesExpiredAndFutureData() throws Exception {
        ...
        assertThrows(ResponseStatusException.class,
                () -> new MaxLaunchDataService("other-token", new ObjectMapper()).userId(valid));
    }

    @Test
    void builtInLaunchReturnsOnlyAuthenticatedUsersSavedGroup() throws Exception {
        var prefs = mock(UserPreferenceService.class);
        when(prefs.selectedGroupId(MessengerType.MAX, "7")).thenReturn(Optional.of(42L), Optional.empty());
        var mvc = standaloneSetup(new MaxMiniAppController(validator, prefs)).build();
        String body = new ObjectMapper().writeValueAsString(
                new MaxMiniAppController.LaunchRequest(signed(Instant.now().getEpochSecond(), "{\"id\":7}")));
        mvc.perform(post("/api/public/max/group").contentType("application/json").content(body))
                .andExpect(status().isOk()).andExpect(header().string("Cache-Control", "no-store"))
                .andExpect(jsonPath("$.groupId").value(42));
        mvc.perform(post("/api/public/max/group").contentType("application/json").content(body))
                .andExpect(status().isOk()).andExpect(content().json("{\"groupId\":null}"));
        verify(prefs, times(2)).selectedGroupId(MessengerType.MAX, "7");
        clearInvocations(prefs);
        mvc.perform(post("/api/public/max/group").contentType("application/json")
