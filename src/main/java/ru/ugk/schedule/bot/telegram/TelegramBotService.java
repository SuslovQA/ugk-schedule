...
public class TelegramBotService {
    ...
    public void poll(){
        ...
        try{
            ...
            for(JsonNode u: root.path("result")){ offset=Math.max(offset,u.path("update_id").asLong()+1); handle(u); }
        }catch(Exception e){ System.err.println("Telegram polling: "+e.getMessage()); }
    }
    ...
    private void showLevels(long chatId){ send(chatId,"Выберите уровень образования", callbackRows(catalog.activeLevels().stream().map(x->new Btn(x.getName(),"L:"+x.getId())).toList())); }
    private void showCourses(long chatId,Long levelId){ send(chatId,"Выберите курс", callbackRows(catalog.activeCourses(levelId).stream().map(x->new Btn(x.getName(),"C:"+x.getId())).toList())); }
    private void showGroups(long chatId,Long courseId){ send(chatId,"Выберите группу / направление", callbackRows(catalog.activeGroups(courseId).stream().map(x->new Btn(x.getName(),"G:"+x.getId())).toList())); }
