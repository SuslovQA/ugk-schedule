...
public class AdminController {
    ...
    @PostMapping("/levels")
    public String saveLevel(@RequestParam(required = false) Long id, @RequestParam String name, @RequestParam Integer maxCourses,
    ...
    public String deleteLevel(@PathVariable Long id, RedirectAttributes ra) {
        try {
            catalog.deleteLevel(id);
        } catch (Exception e) {
            ra.addFlashAttribute("error", "Нельзя удалить: есть связанные данные");
        }
        return "redirect:/admin";
    }

    @PostMapping("/courses")
    public String saveCourse(@RequestParam(required = false) Long id, @RequestParam Long levelId, @RequestParam Integer number,
    ...
    @PostMapping("/groups")
    public String saveGroup(@RequestParam(required = false) Long id, @RequestParam Long courseId, @RequestParam String name,
                            @RequestParam(defaultValue = "false") boolean active, RedirectAttributes ra) {
        try {
    ...
    public String deleteGroup(@PathVariable Long id, RedirectAttributes ra) {
        try {
            ...
            ra.addFlashAttribute("error", "Нельзя удалить: есть расписание");
        }
        return "redirect:/admin";
    }
}
