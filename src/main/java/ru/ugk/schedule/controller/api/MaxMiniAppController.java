...
import ru.ugk.schedule.service.MaxLaunchDataService;
import ru.ugk.schedule.service.UserPreferenceService;

@RestController
@RequestMapping("/api/public/max")
public class MaxMiniAppController {
    private final MaxLaunchDataService launchData;
    private final UserPreferenceService preferences;

    public MaxMiniAppController(MaxLaunchDataService launchData, UserPreferenceService preferences) {
    ...
    public ResponseEntity<GroupResponse> group(@RequestBody LaunchRequest request) {
        ...
        Long groupId = preferences.selectedGroupId(MessengerType.MAX, userId).orElse(null);
