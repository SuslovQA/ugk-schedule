...
public class AdminBootstrapConfig {
    @Bean CommandLineRunner createAdmin(AdminUserRepository repo, PasswordEncoder encoder,
                                        @Value("${app.admin.username:admin}") String username,
                                        @Value("${app.admin.password:admin123}") String password){
        return args -> {
            if(repo.findByUsername(username).isEmpty()){
                AdminUser u = new AdminUser(); u.setUsername(username); u.setPasswordHash(encoder.encode(password)); repo.save(u);
